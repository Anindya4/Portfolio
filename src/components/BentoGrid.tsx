...
const ProjectBentoCard: React.FC<ProjectCardProps> = ({ project, index, onSelect, renderMock }) => {
  ...
  return (
    ...
    >
      {/* Interactive mouse-following white spotlight glow */}
      <div
        className="pointer-events-none absolute -inset-px rounded-[1.25rem] transition-opacity duration-300 z-0"
        style={{
          opacity: isHovered ? 1 : 0,
          background: `radial-gradient(420px circle at ${mousePos.x}px ${mousePos.y}px, rgba(255, 255, 255, 0.09), transparent 70%)`,
        }}
      />

      {/* Dynamic white border spotlight highlight */}
      <div
        className="pointer-events-none absolute -inset-px rounded-[1.25rem] transition-opacity duration-300 z-0 border border-white/40"
        style={{
          opacity: isHovered ? 1 : 0,
          maskImage: `radial-gradient(260px circle at ${mousePos.x}px ${mousePos.y}px, black, transparent 80%)`,
          WebkitMaskImage: `radial-gradient(260px circle at ${mousePos.x}px ${mousePos.y}px, black, transparent 80%)`,
        }}
      />
  ...
  return (
    <section id="projects" className="py-24 md:py-32 relative border-b border-white/5">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {/* Bento Grid Container with generous gap */}
