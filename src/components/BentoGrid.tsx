          ...
          <AnimatePresence>
            {filteredProjects.map((project, index) => (
              <motion.div
                ...
                className={`group ${project.gridSpan.desktop} ${project.gridSpan.height || 'min-h-[380px]'} bento-card cursor-pointer flex flex-col justify-between p-6 sm:p-8`}
              >
                {/* Content */}
                <div className="relative z-10 w-full h-full flex flex-col justify-between">
                  ...
                  <div className="my-5 w-full flex-1 flex items-center justify-center">
                    <div className="w-full h-full min-h-[180px] sm:min-h-[210px] rounded-xl overflow-hidden transition-transform duration-300 group-hover:scale-[1.01]">
                      {renderCardMock(project.mockType)}
                    </div>
                  </div>
